...
class Simulator(gym.Env):
    ...
    def __init__(
        ...
        # Hilfsvariable, um z.B. Trim oder Rad‐Abweichung zu speichern:
        self._dyn_trim_left = 0.0
        self._dyn_trim_right = 0.0
    ...
    def reset(self):
        ...
        # Dynamics randomization: Randomize the trim for left and right wheels
        if self.dynamics_rand:
            self._dyn_trim_left = self.np_random.uniform(0.9, 1.1)
            self._dyn_trim_right = self.np_random.uniform(0.9, 1.1)
            #####################Print um zu überprüfen, ob dynamics Randomization funktioniert
            #print(f"[DEBUG] Dynamics trim left: {self._dyn_trim_left}, trim right: {self._dyn_trim_right}")

        else:
            self._dyn_trim_left = 1.0
            self._dyn_trim_right = 1.0
        ...
        if self.domain_rand:
            ...
            ###################weiterer Print zur Überprüfung ob DR funktioniert
            #print(f"[DEBUG] Domain Randomization Settings: {self.randomization_settings}")
        ...
        # Randomize camera properties if camera_rand is True
        if self.camera_rand:
            self.cam_height *= self.np_random.uniform(0.95, 1.05)
            self.cam_angle[0] *= self.np_random.uniform(0.95, 1.05)
            #################weiterer Print zur Überprüfung von camera_rand
            #print(f"[DEBUG] Camera height: {self.cam_height}, Camera angle: {self.cam_angle[0]}")
        ...
        for attempt in range(MAX_SPAWN_ATTEMPTS):
            i, j = tile['coords']
            # Wähle zufällig eine Position innerhalb des Tiles
            ...
            # Wähle einen zufälligen Winkel
            propose_angle = self.np_random.uniform(0, 2 * math.pi)

            # Überprüfe, ob die Position zu nah an einem Objekt liegt
            if self._inconvenient_spawn(propose_pos):
                continue

            # Überprüfe, ob es sich um eine gültige Pose handelt
            if not self._valid_pose(propose_pos, propose_angle, safety_factor=1.3):
                continue

            # Versuche, eine gültige Lane-Position zu erhalten
            try:
                lp = self.get_lane_pos2(propose_pos, propose_angle)
            except NotInLane:
                continue  # Diese Position liefert keine gültige Fahrspur

            # Optional: Überprüfe, ob der Winkel innerhalb eines akzeptablen Bereichs liegt.
            if not (-self.accept_start_angle_deg < lp.angle_deg < self.accept_start_angle_deg):
                continue

            # Wenn alles passt, setze die Startposition
            self.cur_pos = propose_pos
            self.cur_angle = propose_angle
            logger.info('Starting at %s, %s' % (self.cur_pos, self.cur_angle))
            break
        else:
            raise Exception('Could not find a valid starting pose after %s attempts' % MAX_SPAWN_ATTEMPTS)

        # Erzeuge das erste Kamera-Bild und gib es zurück
        obs = self.render_obs()
        return obs
    ...
    def _interpret_map(self, map_data: dict):
        if 'tile_size' not in map_data:
            raise ValueError('Map data must include explicit tile_size.')
        ...
        for j, row in enumerate(tiles):
            if len(row) != self.grid_width:
                raise Exception("Each row of tiles must have the same length")
            for i, tile in enumerate(row):
                ...
                self._set_tile(i, j, tile_dict)

                # Nur für drivable Tiles: versuche, die Kurvendaten zu laden
                if drivable:
                    curves = self._get_curve(i, j)
                    if curves is None or len(curves) == 0:
                        # Falls keine Kurven gefunden werden, markiere Tile als nicht drivable
                        tile_dict['drivable'] = False
                    else:
                        tile_dict['curves'] = curves
                        self.drivable_tiles.append(tile_dict)

        self.mesh = ObjMesh.get('duckiebot')
        self._load_objects(map_data)

        # Optional: setze start_tile, wenn in der Map angegeben
    ...
    def get_grid_coords(self, abs_pos):
        ...
        # Prüfen auf NaN oder unendliche Werte
        if not math.isfinite(x) or not math.isfinite(z):
            print("[WARN] Detected NaN position -> defaulting to (0,0)!")
            x = 0.0
            z = 0.0  # Standardwert setzen oder andere Logik
    ...
    def update_physics(self, action, delta_time=None):
        ...
        # Update wheel velocities with dynamics randomization
        Vl = action[0] * self.robot_speed * self._dyn_trim_left
        Vr = action[1] * self.robot_speed * self._dyn_trim_right
        self.wheelVels = np.array([Vl, Vr])

        prev_pos = self.cur_pos

        # Update the robot's position
        self.cur_pos, self.cur_angle = _update_pos(
            self.cur_pos,
            self.cur_angle,
            self.wheel_dist,
...
def _update_pos(pos, angle, wheel_dist, wheelVels, deltaTime):
    ...
    pos = np.array([npx, py, npz], dtype=object)
