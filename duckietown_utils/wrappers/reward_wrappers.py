...
class DtRewardPosAngle(gym.RewardWrapper):
    ...
    def reward(self, reward):
        ...
        angle_narrow_reward = float(angle_narrow_reward)  # oder angle_narrow_reward.item()
        angle_wide_reward   = float(angle_wide_reward)    # oder angle_wide_reward.item()
    ...
    def step(self, action):
        ...
        #print(f"[DEBUG] Type of info: {type(info)}")  # Überprüfe den Typ von info
        #print(f"[DEBUG] Keys in info (if dict): {info.keys() if isinstance(info, dict) else 'N/A'}")
        #if not isinstance(info, dict):
        #    print(f"[ERROR] info is not a dict: {info}")
